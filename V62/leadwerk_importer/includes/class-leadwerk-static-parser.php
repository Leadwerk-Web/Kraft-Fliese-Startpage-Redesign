<?php
/**
 * Convert one static Kraft Fliesen HTML document into a render template and
...
class Leadwerk_Static_Parser {
	...
	private function collect_media_fields( &$payload ) {
		$images = $this->xpath->query( '//body//img[@src]' );
		foreach ( iterator_to_array( $images ) as $node ) {
			if ( ! $node instanceof DOMElement ) {
				continue;
			}
			$source = $this->normalize_source_path( $node->getAttribute( 'src' ) );
			if ( '' === $source || $this->is_external_url( $source ) ) {
				continue;
			}
			$key = $this->stable_key( 'image', $node );
			$node->setAttribute( 'data-leadwerk-media', $key );
