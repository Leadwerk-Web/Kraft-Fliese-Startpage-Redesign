<?php
/**
 * Batched static-site importer for Kraft Fliesen.
 *
...
class Leadwerk_Importer {
		...
		for ( $i = $cursor; $i < $end; ++$i ) {
			...
			if ( $this->apply ) {
				...
				if ( $id > 0 ) {
					...
					$state['warning_count']++;
					$this->add_log( $state, 'Medya içe aktarılamadı: ' . $path, 'warning' );
				}
			} else {
				$state['success_count']++;
	...
	private function run_finalize( $state ) {
		if ( $this->apply ) {
			update_option( 'blogname', sanitize_text_field( (string) ( $this->manifest['site_title'] ?? 'Kraft Fliesen GmbH' ) ) );
			update_option( 'blogdescription', sanitize_text_field( (string) ( $this->manifest['site_tagline'] ?? '' ) ) );
			update_option( 'permalink_structure', '/%postname%/' );
			$form_query = new WP_Query( array( 'post_type' => 'wpforms', 'post_status' => 'publish', 'posts_per_page' => 1, 'fields' => 'ids', 'title' => 'Kraft Fliesen Kontaktformular' ) );
