<?php
/**
 * Generic content schema for Kraft Fliesen pages.
 *
		...
		foreach ( array( 'text_items', 'link_items', 'media_items' ) as $bucket ) {
			...
			foreach ( $rows as $row ) {
				...
				if ( 'text_items' === $bucket ) {
					$base['content'] = wp_kses_post( (string) ( $row['content'] ?? '' ) );
				} elseif ( 'link_items' === $bucket ) {
					$base['href'] = self::sanitize_link_value( (string) ( $row['href'] ?? '' ) );
				} else {
					$base['kind']          = in_array( (string) ( $row['kind'] ?? '' ), array( 'image', 'background' ), true ) ? (string) $row['kind'] : 'image';
