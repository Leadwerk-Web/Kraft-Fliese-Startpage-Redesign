...
function leadwerk_theme_render_page( $post_id ) {
	...
	foreach ( iterator_to_array( $xpath->query( '//*[@data-leadwerk-text]' ) ) as $node ) {
		...
		if ( isset( $text[ $key ] ) ) {
			leadwerk_theme_set_inner_html( $dom, $node, wp_kses_post( (string) $text[ $key ]['content'] ) );
		}
	}
	...
	foreach ( iterator_to_array( $xpath->query( '//*[@data-leadwerk-media]' ) ) as $node ) {
		$key = $node->getAttribute( 'data-leadwerk-media' );
		if ( ! isset( $media[ $key ] ) ) {
			continue;
		}
		$row = $media[ $key ];
	...
	foreach ( iterator_to_array( $xpath->query( '//*[@data-leadwerk-wpforms]' ) ) as $node ) {
		...
		if ( '' !== $form && shortcode_exists( 'wpforms' ) ) {
			...
			$message = current_user_can( 'manage_options' ) ? 'WPForms Formular-ID unter Ayarlar → Kraft Fliesen Website alanına eklenmeli.' : 'Das Kontaktformular ist momentan nicht verfügbar. Bitte rufen Sie uns an oder schreiben Sie eine E-Mail.';
